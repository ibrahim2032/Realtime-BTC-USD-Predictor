...
def interpolate_missing_candles(
    ...
    """
    # Python trick!
    # Use inplace to avoid copying the data, which should be more efficient
    ...
    # complete list of timestamps to have rows in our dataframe
    ...
    # Forward fill the product_id as well
    ohlc_data['product_id'].ffill(inplace=True)

    # reset the index
    ohlc_data.reset_index(inplace=True)

    # Make sure no missing datetimes
...
def create_target_metric(
    ...
    # Drop blank target values before returning the dataframe
